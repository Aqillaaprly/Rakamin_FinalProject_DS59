...
def run():
    ...
    with col_left:
        ...
        dept = st.selectbox("", list(department_jobs.keys()))
        st.subheader("Job Title")
        job  = st.selectbox("", department_jobs[dept])

        st.write("----")

        time_w = st.number_input("Time Weight", 0.0,1.0,0.3,step=0.01)
        cost_w = st.number_input("Cost Weight", 0.0,1.0,0.3,step=0.01)
        oar_w  = st.number_input("OAR Weight",  0.0,1.0,0.3,step=0.01)
    ...
    with col_right:
        ...
        if predict and valid:
            ...
            for src in sources:
                ...
                X = generate_features(dept, job, src,
            ...
            with c1:
                ...
                st.markdown(
                    f"<h3 style='color:#b00000;'>{best_source}</h3>",
                    unsafe_allow_html=True
                )
